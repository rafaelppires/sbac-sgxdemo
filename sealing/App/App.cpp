...
int SGX_CDECL main(int argc, char *argv[]) {
    ...
    if( argc > 1 ) {
        char buff[1000];
        int ret;
        if (SGX_SUCCESS == ecall_seal_secret(global_eid, &ret, argv[1], buff, sizeof(buff))) {
            std::ofstream out(fname);
            if( ret > 0 )
                out << std::string(buff,ret);
        } else
        ...
        if (in.read(buff.data(), file_size)) {
            if( SGX_SUCCESS != ecall_unseal(global_eid, buff.data(), file_size) )
                printf("ecall_unseal: error\n");
        }
    }
